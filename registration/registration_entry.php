...
if (isset($category)) {
    if ($flag['fullname'] == true && $flag['email'] == true && $flag['contactno'] == true && $flag['gender'] == true && $flag['userid'] == true && $flag['usertype'] == true && $flag['category'] == true && $flag['password'] == true && $flag['confpassword'] == true) {
        $stmt = $conn->prepare("INSERT INTO pending_requests (usr_id, usr_name, usr_contact_no, usr_gender, usr_email, usr_password, usr_type, usr_stud_category) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssss", $userid, $fullname, $contactno, $gender, $email, $password1, $usertype, $category);
    }
} else {
    if ($flag['fullname'] == true && $flag['email'] == true && $flag['contactno'] == true && $flag['gender'] == true && $flag['userid'] == true && $flag['usertype'] == true && $flag['password'] == true && $flag['confpassword'] == true) {
        $stmt = $conn->prepare("INSERT INTO pending_requests (usr_id, usr_name, usr_contact_no, usr_gender, usr_email, usr_password, usr_type) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssss", $userid, $fullname, $contactno, $gender, $email, $password1, $usertype);
    }
}
...
if (isset($stmt)) {
    if ($stmt->execute()) {
        ...
        $flag['success'] = false;
        $flag['errorcode'] = $stmt->errno;
        $flag['errormessage'] = $stmt->error;
    }
    $stmt->close();
}
